class GeometryDash {
    ...
    setupAudio() {
        this.sounds = {
            jump: new Audio('data:audio/wav;base64,UklGRigAAABXQVZFZm10IBIAAAABAAEAQB8AAEAfAAABAAgAZGF0YQ'),
            score: new Audio('data:audio/wav;base64,UklGRigAAABXQVZFZm10IBIAAAABAAEAQB8AAEAfAAABAAgAZGF0YQ'),
            powerup: new Audio('data:audio/wav;base64,UklGRigAAABXQVZFZm10IBIAAAABAAEAQB8AAEAfAAABAAgAZGF0YQ'),
            gameover: new Audio('data:audio/wav;base64,UklGRigAAABXQVZFZm10IBIAAAABAAEAQB8AAEAfAAABAAgAZGF0YQ')
        };
    }
    ...
    draw() {
        ...
        // РОТ (дополнительное задание)
        this.ctx.fillStyle = '#000000';
        if (this.player.mouthOpen) {
            // Открытый рот
            this.ctx.fillRect(-this.player.width/8, this.player.height/8, this.player.width/4, 6);
        } else {
            // Закрытый рот - улыбка
            this.ctx.beginPath();
            this.ctx.arc(0, this.player.height/8, this.player.width/8, 0, Math.PI);
            this.ctx.stroke();
        }
    ...
    gameOver() {
        ...
        if (finalScoreElement) {
            finalScoreElement.textContent = `Счет: ${this.score}`;
        }
    ...
    shareScore() {
        const text = `🎮 Я набрал ${this.score} очков в Geometry Dash - Technical Edition!`;
        if (navigator.share) {
            navigator.share({
                title: 'Мой результат в Geometry Dash',
                text: text,
